...
float4 ObjectToWorldDistort3dNP(in float3 pos)
{
	float u = (pos.x + 0.5);
	float v = 1 - (pos.y + 0.5);
	float w = pos.z + 0.5;
	float3 o = _ftl * (1 - u) * (1 - v) * (1 - w) + _ftr * u * (1 - v) * (1 - w) + _fbl * (1 - u) * v * (1 - w) + _fbr * u * v * (1 - w) +
				_btl * (1 - u) * (1 - v) * w + _btr * u * (1 - v) * w + _bbl * (1 - u) * v * w + _bbr * u * v * w;
//	float4 op = mul(mul(UNITY_MATRIX_P, float4(o, 1.0));
	return (float4(o, 1.0));
}
...
{
	float u = (pos.x + 0.5);
	float v = 1.0 - (pos.y + 0.5);
	float3 o = _ftl * (1.0 - u) * (1.0 - v) + _ftr * u * (1.0 - v) + _fbl * (1.0 - u) * v + _fbr * u * v;
